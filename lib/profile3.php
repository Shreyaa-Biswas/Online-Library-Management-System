...
if (isset($_SESSION['user_id'])) {
    ...
    // Query to get student details along with the count of currently borrowed books
    $sql = "SELECT 
            student.first_name, 
            student.last_name, 
            student.email, 
            student.phone, 
            member.member_id, 
            COUNT(DISTINCT book.Book_ID) AS no_of_current_books,  -- Count of currently borrowed books
            COUNT(DISTINCT borrowed.Book_ID) AS total_books,       -- Total number of books borrowed (historical)
            member.admin_username 
        FROM 
            student 
        INNER JOIN member ON student.id = member.student_id 
        LEFT JOIN book ON book.Member_ID = member.member_id
        LEFT JOIN borrowed ON borrowed.Member_ID = member.member_id
        WHERE 
            User_ID = '$user_id'
        GROUP BY
            student.id, member.member_id, member.admin_username";
    ...
    if (mysqli_num_rows($result) != 0) {
        ...
        $phn = $student_data['phone'];
        $memid = $student_data['member_id'];
        $nob = $student_data['no_of_current_books'];
        $total_books = $student_data['total_books'];
        $adusr = $student_data['admin_username'];
    } else {
...
<body>
    <nav class="navbar navbar-inverse">
        <div class="container-fluid">
            ...
            <ul class="nav navbar-nav navbar-right">
                ...
                <li><a href="book list.php?user_id=<?php echo $user_id; ?>" style="margin-left: 20px;" class="right-buttons">Books List</a></li>
                <!-- Add this line for the Books page -->
                <?php
    ...
    <div class="faded-box">
        ...
        <label for="No. of Currently Borrowed Books">Currently Borrowed Books:</label>
        <span><?php echo $nob; ?></span><br>
        <label for="Total Books Borrowed">Total Books Borrowed:</label>
        <span><?php echo $total_books; ?></span><br>
